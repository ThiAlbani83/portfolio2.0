            ...
            <div className='grid grid-cols-2 w-full gap-10'>
                {skills.map((val, index) => (
                    <div key={index} className='flex flex-col items-center justify-center gap-3'>
                        ...
                        <div className="w-[35vw] bg-gray-200 rounded-full">
                            <div className={`bg-blue-600 text-xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full w-[${val.progress}]`}>{val.progress}</div>
                        </div>
                    </div>
                ))}
